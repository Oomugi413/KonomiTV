...
作成日: 2026-07-11
更新日: 2026-07-21（iOS 27 beta 4 対応を追記）

比較対象:

- 変更前: `.codex/PlayerController_20260612.ts`（2195行）
- 現在版: `client/src/services/player/PlayerController.ts`（2676行）
- iOS 27 beta 3 時点のコピー: `.codex/PlayerController_27.ts`（2665行）

現在版は変更前より481行増えている。以下の行番号は、すべて現在版 `client/src/services/player/PlayerController.ts` の行番号である。
...
2026-08-04追記: mpegts.js更新後にPlayerControllerを差し戻すと、一時停止直後の再生で後続フレームが止まることをmacOS Safariで再現した。更新内容はAAC timestamp gap補正のみでMSE in Workerの状態ずれは対象外だったため、本項目のSafari Live向け無効化を再適用した。
...
2026-08-04追記: 正常再生中の「一時停止→即再生」でもDPlayerのdirect live syncとWebKitのデコード再開が競合し得るため、本項目を再適用した。
...
2026-08-04追記: 再現時に再読み込み後の `currentTime` が非finiteである一方、buffered rangeが存在する状態を再確認したため、本項目を再適用した。
...
## 11. startup stall recovery（iOS 27 beta 3 時点で変更）
...
2026-08-04追記: 本項目の考え方を強化し、Safari Liveでは通常のbuffering recoveryから追加の `pause()` を完全に除外した。次の実フレームを待ち、非破壊的な `play()` 再要求、それでも復旧しない場合だけライブ末尾付近へseekする。これにより、ユーザーの `play()` を内部 `pause()` が中断して発生していた `AbortError` を防ぐ。
...
## 14. 初回 `play()` を起動フローから非ブロッキング化（iOS 27 beta 3 時点で変更）
...
2026-08-04追記: PlayerController差し戻しによって再び `await attemptPlay()` に戻っていたため、本項目を再適用した。

## 15. `on_canplay()` の診断・Safari固有処理（iOS 27 beta 3 時点で変更、iOS 27 beta 4 時点で追加修正）
...
- Safariではbuffer調整中に `playbackRate=0` を設定しない。
- Safari以外だけ `playbackRate` を1へ戻し、完了時状態を記録。
- SafariでMSE初期化後に非破壊的な `video.play()` を再要求。
- 実フレームが出なければloading UIを維持し、ONAirならPlayerController再起動を要求。

SafariではMSE開始中の `play()` が保留されている状態で `playbackRate=0` を設定すると、WebKitの再生開始自体を止める可能性があるため分岐した。

### iOS 27 beta 4 時点の追加修正

- 1654〜1672行: Safari以外では従来どおり `playbackRate=0` にして、bufferが目標秒数へ到達するまで待つ。
- 1664〜1669行: buffer待機中にPlayerControllerが破棄された場合、古い非同期処理を終了する。
- 1674〜1683行: Safariではbufferの目標秒数到達を待たず、項目9の実フレーム提示検証へ進む。
- 1690〜1696行: Safariの再 `play()` を「buffer確保後」ではなく「MEDIA_INFOまたはcanplayによるMSE初期化後」に実行することを明確化した。

iOS 27 beta 4のSafariでは、通常再生中の `currentTime` と `buffered.end()` がほぼ同じ速度で進み、再生bufferが約0.8秒のままになる場合がある。iOS 27 beta 3時点の分岐ではSafariを停止させずにbuffer目標値を待っていたため、待機ループを永久に抜けられず、その後の一時ミュート解除へ到達できなかった。

Safariではbuffer秒数を起動完了の必須条件から外し、既存の `requestVideoFrameCallback()` による実フレーム提示検証を成功判定として利用する。これにより、iOS 27 beta 3向けに導入した「Safariでは `playbackRate=0` を設定しない」という方針を維持しながら、映像再生中に音声だけがミュートされたままになる問題を解消する。

### 2026-08-04 mpegts.js更新後の再適用

PlayerController差し戻し後のSafariで、正常再生中に一時停止してすぐ再生すると後続映像・音声が停止する症状を確認した。Safariでは引き続き `playbackRate=0` と起動buffer目標待ちを使わず、MSE初期化後に非破壊的な `play()` を要求して実フレームを確認する処理を再適用した。
